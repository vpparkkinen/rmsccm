...
# Fuzzy-set variant of random noise benchmarking tests.
...
# con=cov=0.75 and con=cov=0.8,
# 3. scanning model fit thresholds for largest values that produces non-null results and selecting by best model fit, and 
# 4. selecting by FR-score.

# preparations for test series

# NOTE: running the full test battery with 1000 data sets might take a long time (on a typical desktop, hours). 
# To get a result faster, reduce the number assigned to n.sets.
...
fuzzvals <- list(seq(0, 0.2, by = 0.01), seq(0, 0.6, by = 0.01))

fitfloor <- c(0.7, 0.75, 0.8) # fit floor for whole series, convention1, convention2
setlist <- list(tcomp, samplesize, fuzzfactor)

settings <- expand.grid(setlist) 
colnames(settings) <- c("tcomp", "ssize", "fuzz") 

results_fuzz <- vector("list", nrow(settings))
inspect_fuzz <- vector("list", nrow(settings))

set.seed(21) #seed for selecting seeds
seeds <- sample(.Machine$integer.max, nrow(settings))

nfacs <- 6 # no. of factors
comple <- 2:3 # max complexity
...
cat("test", s, "of", nrow(settings),"--", "target outcomes:", settings[s,1], " ",
    ...
    "fuzz:", min(fuzzvals[[settings[s,3]]]), "to", max(fuzzvals[[settings[s,3]]]), "\n")
...
alldata <- allCombs(c(rep(2,nfacs)))-1

# select rows that match the target
testdata <- lapply(lapply(target, function(x) selectCases(x, alldata)), ct2df) 
...
# create noisy data 
Noisedata <- lapply(testdata, function(x) makeFuzzy(x, fuzzvalues = fuzzvals[[settings[s,3]]]))

nratio <- ifelse(settings[s,3]==1, "low", "high")

# perform analyses
 
# quote calls (in order to save them with results later)
...
call_reanalysis <- quote(frscore(x = Noisedata[[i]], type = c(1, fitfloor[1], -0.1), inus.only = T, rm.dup.factors = F, rm.const.factors=F,
                                 ...
                                 allconcov = T, normalize =T, ncsf = 50))
...
# analyze data sets with conventional fit threshold options
...
# check if correct model is found in _all_ the results per data set
correct_in_raw <- lapply(ress, function(y) 
  lapply(y, function(x)if (is.na(x$correct)){FALSE}else{any(x$correct)}))

# check if correct found or no solutions at all per data set -- fallacy free 
...
# first take only those models produced by the highest fit threshold settings
...
# select MaxFit models in the top 98 percentile as measured by model con*cov
...
# select models with top robustness score from results of re-analysis series
...
ffree_in_FRscore <- lapply(FRscore_selected, function(y) is.null(y) || any(y$correct) || is.na(y))
...
# save selected results plus additional info for each approach for test s
inspect_fuzz[[s]] <- allscores 
...
# ratio of correct found in per data set in _all_ models returned in a re-analysis series 
...
# correctness ratios in selected models
sel_cor_ratios <- c(avg_cor_FRscore, 
                    avg_cor_convs[[1]], avg_cor_convs[[2]],
                    avg_cor_MaxFit)

# fallacy free ratios in selected
...
results_fuzz[[s]] <- data.frame("selection"=c("robustness", "convention 0.75",
                                              'convention 0.8', 
                                              'max fit threshold'),
...
# store the above
results_all_fuzz <- rbind.fill(results_fuzz)



# summarise by mean the result columns over all test settings
results_all_means_fuzz <- results_all_fuzz %>% group_by(selection) %>% 
...
results_all_means_fuzz


# plots

toplot <- results_all_fuzz
...
k1$lab <- rep(c(rep("conv 0.75", 2), rep("conv 0.8", 2),  rep("maxfit", 2), rep("robust", 2)), 3)
k1$noi.ratio <- as.character(k1$noi.ratio)

plot1_fuzz <- ggplot(k1, aes(x = noi.ratio, y = value, group = selection, fill = variable)) +
  geom_bar(stat="identity", position = position_dodge(width = .8), width=.7) +
  geom_text(aes(x = noi.ratio, y = pos, label = lab, vjust = -3), position = position_dodge(0.7), show.legend = F) +
  scale_fill_grey(start = 0, end = 0.6) +
  theme_bw()+
  #scale_fill_manual(values = c(2,.5,.2))+
  
  # theme(legend.title=element_text(factor))+
  theme(plot.title = element_text(size = 6))+
  scale_x_discrete(name ="noise ratio")+ylim(0, 1.1) + ylab("correctness / fallacy freeness") +
...
#k2$lab <- rep(c(rep("conv 0.75", 2), rep("conv 0.8", 2), rep("robust", 2)), 3)
k2$lab <- rep(c(rep("conv 0.75", 2), rep("conv 0.8", 2),  rep("maxfit", 2), rep("robust", 2)), 3)
k2$t.asfs <- as.character(k2$t.asfs)
plot2_fuzz <- ggplot(k2, aes(x = t.asfs, y = value, group = selection, alpha = variable)) +
  geom_bar(stat="identity", position = position_dodge(width = .8), width=.7) +
  geom_text(aes(x = t.asfs, y = pos, label = lab, vjust = -3), position = position_dodge(0.7), show.legend = F) +
  scale_fill_grey(start = 0, end = 0.6) +
  theme_bw() +
  theme(plot.title = element_text(size = 6)) +
  scale_x_discrete(name ="target asfs")+ylim(0, 1.1) + ylab("correctness / fallacy freeness") +
  scale_alpha_manual(values=c(0.6, .06, .9)) +
...
tp_N_mean <- tp_N %>% group_by(selection, N) %>% 
  summarise(corr.selected=mean(corr.selected), ffree = mean(ffree.selected), 
            completeness = mean(completeness))
tp_N_mean$sp <- rep(c(rep(samplesize[1], 2), rep(samplesize[2], 2)), nrow(results_all_means_fuzz))
...
nplot_fuzz <- ggplot(k3, aes(x = sp, y = value, group = selection, alpha = variable)) +
...
plot3_fuzz <- ggdraw(nplot_fuzz) + ngrob 
plot3_fuzz


# plot 4 -- averages 

selsols <- results_all_means_fuzz[,c(1,6)]
toplot_all <- results_all_means_fuzz
...
plotsel_fuzz <- ggplot(k4, aes(x = variable, y = value, fill = selection)) +
  geom_bar(stat="identity", position = position_dodge(width = .8), width=.7) +
...
plot4_fuzz <- ggdraw(plotsel_fuzz) + tbl
plot4_fuzz
