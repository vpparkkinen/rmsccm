...
# Reproduces the benchmark results for class I non-random noise condition,
...
# set the number of noise rows that will be identical (to simulate systematic noise)
noiserepeats <- round(nrow(testdata[[1]]) * nratio * 0.7)
...
plot2 <- ggplot(k2, aes(x = t.asfs, y = value, group = selection, alpha = variable)) +
  ...
  scale_alpha_manual(values=c(0.6, .06, .9),
                     label = c("correctness", "fallacy-freeness", "completeness")) +
