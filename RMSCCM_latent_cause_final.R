...
# Reproduces the benchmark results for class II non-random noise condition,
...
# selected models by each approach stored here for later inspection
inspect <- vector("list", nrow(settings)) 
...
for(s in 1:nrow(settings)){
  ...
  # preparation for removing one exogenous factor from data 
  
  target_asfs <- cna:::extract_asf(target)
  
  target_asf_lhss <- lapply(target_asfs, cna:::lhs)
  
  target_asf_lhss <- lapply(target_asf_lhss, toupper)
  
  target_lhs_factors <- lapply(target_asf_lhss, function(x) gsub("[^[:upper:]]", "", x))
  
  target_lhs_factors <- lapply(target_lhs_factors, function(x) paste(x, collapse = ""))
 
  target_lhs_factors <- lapply(target_lhs_factors,
                               function(x) paste(unique(strsplit(x, "")[[1]]), collapse = ""))
  
  outcome_factors <- if(length(outcome) > 1){paste(outcome, collapse = "|")} else {outcome}
  
  exo_facs <- lapply(target_lhs_factors, function(x) gsub(outcome_factors, "", x))
  
  # choose a factor to be removed from ideal data
  confounder <- lapply(exo_facs, function(x) sample(strsplit(x, "")[[1]], 1))
  ...
  for (i in 1:length(testdata)){
    Noisedata[[i]] <- testdata[[i]][,-which(colnames(testdata[[i]]) == confounder[[i]])]
  }
...
nplot <- ggplot(k3, aes(x = sp, y = value, group = selection, alpha = variable)) +
  ...
  theme_bw()+
  theme(plot.title = element_text(size = 6), legend.title = element_blank()) +
  scale_x_discrete(name ="sample size multiplier") +
  scale_alpha_manual(values=c(0.6, .06, .9), label = c("correctness", "fallacy-freeness", "completeness")) +
  ylab(element_blank()) +
